...
from modules.stage_b_csi import StageB, ResBlock, AttnBlock, TimestepBlock, FeedForwardBlock
...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    def get_conditions(self, batch: dict, models: Models, extras: Extras, is_eval=False, is_unconditional=False, eval_image_embeds=False, return_fields=None):
        ...
        h_real_normalized, h_imag_normalized, snr_normalized = None, None, None
        h_real_tile, h_imag_tile, snr_tile = None, None, None
        ...
        if images is not None:
            ...
            if is_eval and not is_unconditional:
                ...
                if is_eval:
                    ...
                    effnet_factor = 0.75 # np.random.uniform(0.5, 1) # f64 to f32
                ...
                if not is_eval:
                    ...
                    snr_db = torch.randint(1, 20, (1,)).item()     
                    embeddings_shape = effnet_embeddings.shape
                    batch_size = effnet_embeddings.shape[0]

                    # Convert to bits and modulate (4-QAM)
                    bit_list = tensor2bin(effnet_embeddings.cpu(), bit_width=8)
                    symbols = qam_modulate(bit_list, M=4)
                    ...
                    # Transmit through channel (AWGN, Rician, Rayleigh)
                    noisy_symbols_with_pilots = awgn_channel(symbols_with_pilots, snr_db = snr_db)
                    #noisy_symbols_with_pilots = rician_fading_channel(symbols_with_pilots, snr_db = snr_db)
                    #noisy_symbols_with_pilots = rayleigh_fading_channel(symbols_with_pilots, snr_db = snr_db)

                    # Channel estimation
                    h_estimated = ls_channel_estimation(noisy_symbols_with_pilots[-200:], pilot_symbols, self.device)
                    h_real = h_estimated.real.cpu().item()
                    h_imag = h_estimated.imag.cpu().item()
                    snr_estimated = estimate_snr(noisy_symbols_with_pilots[-200:], pilot_symbols, h_estimated.cpu(), self.device)

                    # Equalization and demodulation
                    noisy_symbols = noisy_symbols_with_pilots[:-200]
                    equalized_symbols = lmmse_equalization(noisy_symbols, h_estimated, snr_estimated, self.device)
                    bit_list_noisy = qam_demodulate(equalized_symbols, M=4, bit_width=8)
                    bit_list_noisy = bit_list_noisy[:effnet_embeddings.numel()]
                    effnet_embeddings = bin2tensor(bit_list_noisy, bit_width=8)
                    effnet_embeddings = effnet_embeddings.reshape(embeddings_shape).to(self.device)

                    # Normalize and preprocess for denoiser
                    noisy_normalized = normalize(effnet_embeddings.cpu().numpy(), stats["noisy_min"], stats["noisy_max"])
                    ...
                    #print(f"Noisy normalized before expansion: {noisy_normalized.shape}")
                    noisy_normalized = np.expand_dims(noisy_normalized, axis=1)
                    #print(f"Noisy normalized after expansion: {noisy_normalized.shape}")
                    ...
                    input_tensor = torch.tensor(input_tensor, device=self.device, dtype=torch.float32)
                    h_real_tile = torch.tensor(h_real_tile, device=self.device, dtype=torch.float32)
                    h_imag_tile = torch.tensor(h_imag_tile, device=self.device, dtype=torch.float32)
                    snr_tile = torch.tensor(snr_tile, device=self.device, dtype=torch.float32)
                    
                    # Denoise 
                    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.bfloat16):
                        denoised_latents = models.denoiser_model(input_tensor)  
                    ...
                    # Reshape denoised_latents to match effnet_latents_SC_FT shape
                    denoised_latents = torch.FloatTensor(denoised_latents).reshape(embeddings_shape).to(self.device)
                    effnet_embeddings = denoised_latents.to(torch.bfloat16)
        else:
        ...
        return {'effnet': effnet_embeddings,
                'clip': conditions['clip_text_pooled'],
                'h_real': h_real_tile if h_real_tile is not None else None,
                'h_imag': h_imag_tile if h_imag_tile is not None else None,
                'snr': snr_tile if snr_tile is not None else None}

    def setup_models(self, extras: Extras, skip_clip: bool = True) -> Models:
        ...
        if self.config.generator_checkpoint_path is not None:
            print(f"self.config.generator_checkpoint_path: {self.config.generator_checkpoint_path}")
            checkpoint = load_or_fail(self.config.generator_checkpoint_path)
            if loading_context is dummy_context:
                generator.load_state_dict(checkpoint if 'state_dict' not in checkpoint else checkpoint['state_dict'], strict=False)
                print("Loaded StageB checkpoint with strict=False, missing keys for channel_context_mapper are initialized with default weights.")
                #generator.load_state_dict(load_or_fail(self.config.generator_checkpoint_path))
            else:
                for param_name, param in (checkpoint.items() if 'state_dict' not in checkpoint else checkpoint['state_dict'].items()):
                #for param_name, param in load_or_fail(self.config.generator_checkpoint_path).items():
                    #if 'channel_context_mapper' not in param_name: # skip missing keys
                    #    set_module_tensor_to_device(generator, param_name, "cpu", value=param)
                    set_module_tensor_to_device(generator, param_name, "cpu", value=param)
                print("Loaded StageB checkpoint to CPU with init_empty_weights, skipping channel_context_mapper keys.")
               
        generator = generator.to(dtype).to(self.device)
        #generator = self.load_model(generator, 'generator')
        
        if generator_ema is not None:
            print("We entered the generator ema")
            if loading_context is dummy_context:
        ...
        # Add denoiser model initialization
        print("Loading Denoiser Model")
        denoiser_model = EmbeddingDenoiseModel().to(self.device)
        denoiser_model.load_state_dict(torch.load('models/denoiser/v3/denoiser_v3.pth', map_location=self.device))
        denoiser_model.eval()

        return self.Models(
            effnet=effnet, stage_a=stage_a,
            generator=generator, generator_ema=generator_ema,
            tokenizer=tokenizer, text_model=text_model, denoiser_model=denoiser_model
        )

    #def setup_stage_b(self, extras: Extras, skip_clip: bool = True) -> Models:
    def setup_stage_b(self, extras: Extras, skip_clip: bool = True):
        #dtype = getattr(torch, self.config.dtype) if self.config.dtype else torch.float32
        dtype = torch.float32
        ...
        if self.config.use_fsdp and FSDP is not None:
            fsdp_auto_wrap_policy = ModuleWrapPolicy([ResBlock, AttnBlock, TimestepBlock, FeedForwardBlock])
            generator = FSDP(generator, **self.fsdp_defaults, auto_wrap_policy=fsdp_auto_wrap_policy, device_id=self.device)
            if generator_ema is not None:
                generator_ema = FSDP(generator_ema, **self.fsdp_defaults, auto_wrap_policy=fsdp_auto_wrap_policy, device_id=self.device)
        ...
        #return generator, self.Models(generator=None, generator_ema=None, effnet=None, stage_a=None, tokenizer=None, text_model=None)
        return generator
