...
class StageB(nn.Module):
    def __init__(self, c_in=4, c_out=4, c_r=64, patch_size=2, c_cond=1280, c_hidden=[320, 640, 1280, 1280],
        ...
        # Channel context mapper for h_real, h_imag, snr
        self.channel_context_mapper = nn.Sequential(
            nn.Conv2d(3, c_hidden[0] * 4, kernel_size=1),  
            nn.GELU(),
            nn.Conv2d(c_hidden[0] * 4, c_hidden[0], kernel_size=1),
            LayerNorm2d(c_hidden[0], elementwise_affine=False, eps=1e-6)
        )
        ...
        nn.init.normal_(self.channel_context_mapper[0].weight, std=0.02)    # channel context
        nn.init.normal_(self.channel_context_mapper[2].weight, std=0.02)    # channel context
    ...
    def gen_c_embeddings(self, clip):
        if clip is None:
            # Return zero embeddings or skip if clip is not used
            return torch.zeros(2, 4, 1280, device=self.clip_mapper.weight.device)
        if len(clip.shape) == 2:
    ...
    def forward(self, x, r, effnet, clip, pixels=None, h_real=None, h_imag=None, snr=None, **kwargs):
        if pixels is None:
        ...
        clip = self.gen_c_embeddings(clip) if clip is not None else self.gen_c_embeddings(None)
        ...
        # Process channel context (h_real, h_imag, snr)
        if h_real is not None and h_imag is not None and snr is not None:
            h_real = h_real[:, :, 0, :, :]  
            h_imag = h_imag[:, :, 0, :, :]  
            snr = snr[:, :, 0, :, :]        
            channel_context = torch.cat([h_real, h_imag, snr], dim=1)  
            x = x + self.channel_context_mapper(
                nn.functional.interpolate(channel_context.float(), size=x.shape[-2:], mode='bilinear', align_corners=True)
            )
